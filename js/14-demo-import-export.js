...
async function carregarDemo(){
  ...
  try{
    ...
    const proc2 = await dbInserir('processos', { numero:p2, empresa:'CHALLENGE', clienteId:c2.id, descricao:'Materiais de construção', dataAbertura:iso(ano,3,3), dataProntidao:iso(ano,3,20), dataEmbarque:'', moeda:'USD', valorMoeda:85000, taxaCambio:'', dataFechCambio:'', statusRecebimento:'Pendente', obs:'' });
    ...
    const car1 = await dbInserir('contasReceber', { processoNumero:p2, empresa:'CHALLENGE', clienteId:c2.id, ref:'PI-2201', moeda:'USD', valor:85000, vencimento:iso(ano,4,30), dataRecebimento:'', valorRecebido:'' });
    ...
    const cap1 = await dbInserir('contasPagar', { processoNumero:p2, empresa:'CHALLENGE', vencimento:iso(ano,3,25), fornecedor:'Transportadora Demo', centroCusto:'LOGISTICA', valor:6200, dataPagamento:'' });
